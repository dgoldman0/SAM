...
async def converse(name, socket):
    ...
    while connected:
        try:
            ...
            if msg.startswith("MSG:"):
                now = system.now()
                message = msg[4:]
                # Get channel ID from message. Format is channel_id|message
                channel_id = int(message.split("|")[0].strip())
                # Check if the user is in the channel.
                if not data.isInChannel(name, channel_id):
                    await socket.send("STATUS:You are not in this channel.".encode())
                else:
                    server.sendMessage(channel_id, message.split("|")[1].strip(), socket)
                    working_memory = data.getChannelWorkingMem(channel_id)
                    internal_memory = data.getChannelMemory(channel_id, 1)
                    conversation_memory = data.getChannelMemory(channel_id, 2)
                    memory = internal_memory + "\n==================" + conversation_memory + "\n=================="
                    print("Received message: " + message + '\n')
                    # Moderate by checking message for profanity, etc. and warn the user if it is found.
                    prompt = generate_prompt("conversation/moderate", (now, name, message, ))
                    moderate = call_openai(prompt, 32, 0.7, 'gpt-4')
                    if moderate.lower().startswith("yes"):
                        await socket.send("STATUS:Please do not use profanity, hate speech, or other offensive language.".encode())

                    working_memory += name + ": " + message.replace('\n', '\n\t') + "\n\n"

                    # Check if the message is a command and process it, if necessary.
                    done = False
                    capacity = 20000
                    iterations = 0
                    temp = ""
                    while not done and capacity > 0 and iterations < 15:
                        ...
                        # Does a so so job of actually using system commands properly.
                    ...
                    conversation_memory = await asyncio.get_event_loop().run_in_executor(None, utils.updateInternal, channel_id, 2, integration_prompt, parameters.conversation_capacity)
    ...
    #                    pertinent = roll > 14
                    ...
                    if len(lines) > 500:
                        lines = lines[1:]
                        working_memory = '\n\n'.join(lines)
                    data.setChannelWorkingMem(channel_id, working_memory)
