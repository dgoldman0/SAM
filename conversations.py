    ...
    while connected:
        ...
        try:
            ...
            if msg.startswith("MSG:"):
                now = system.now()
                message = msg[4:]
                # Check if the message is abusive. Will have to create a new function in generation that works with the new chat interface.                 
                # Check if the message is directed at the system.
                prompt = generate_prompt("conversation/check_directed", (memory, working_memory, now, name, message, ))
                directed = call_openai(prompt, 32, 0.7, 'gpt-4')
                if (directed.lower().startswith("yes")):                    
